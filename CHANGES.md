...
## Unreleased
- Dashboard hero rebuilt on `components/ui/empty.tsx` primitives (`Empty`/`EmptyHeader`/`EmptyMedia`/`EmptyTitle`/`EmptyDescription`): 48px logo, `text-2xl` headline, default-size description. Fixed earlier sizing misses caused by leftover `size-16 md:size-[72px]` and `md:text-5xl` classes overriding the `width`/`height` props and base text size.
